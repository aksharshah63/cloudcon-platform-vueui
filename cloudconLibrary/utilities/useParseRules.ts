import date from "./useDateOperations";
  ...
  function getRules(rules: string) {
    ...
    const ruleDescriptionsArray = rulesArray.map((r) => {
      ...
      if (d["type"] === "static") {
        ...
        if (d["value"])
          d["value"] = date.getDateString(d["value"] as number, dateFormat);
        if (d["value1"])
          d["value1"] = date.getDateString(d["value1"] as number, dateFormat);
        if (d["value2"])
          d["value2"] = date.getDateString(d["value2"] as number, dateFormat);
      }
