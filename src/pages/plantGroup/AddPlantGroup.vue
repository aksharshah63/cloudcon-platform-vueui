...
export const AddPlantGroup = /*#__PURE__*/ defineComponent({
  name: "AddPlantGroup",
  components: {
    MenuSidebar,
    InputText,
      ...
      if (is_error == false) {
        if (token) {
          if (!router.currentRoute.value.params.id) {
            loading.value = true;
            controllers
              .addplantgroup(fname.value)
              .then(() => {
                toast.add({
                  severity: "success",
                  summary: "Success Message",
                  detail: "Plant Group Add Successfully",
                  life: 10000,
                });
                loading.value = false;
                router.push({
            ...
            controllers
              ...
              .then(() => {
                toast.add({
                  severity: "success",
                  summary: "Success Message",
                  detail: "Plant Group Update Successfully",
                  life: 10000,
                });
                loading.value = false;
                router.push({
      ...
      if (router.currentRoute.value.params.id) {
        if (token) {
          ...
          const data = JSON.parse(
            await controllers.getplantgroupbyid(
              Number(router.currentRoute.value.params.id)
            )
          );
    ...
    <div v-else>
      <section class="page-header">
        <h2 class="page-hedaer-title">
          {{ $route.params.id ? "Update Plant Group" : "Add Plant Group" }}
        </h2>
        <div class="header-btn-wrapper">
          <Button
            type="button"
            label="create Plant Group"
