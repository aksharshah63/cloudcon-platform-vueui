...
/* eslint-disable */
export const FormTemplateGroupDashboard = /*#__PURE__*/ defineComponent({
  ...
  setup() {
    ...
    const columns = ref<Record<string, string>[]>([
      ...
      { label: "Templates Count", name: "groups" },
    ]);
    ...
    const formTemplateGroupsData = computed<IFormTemplateGroup[]>(() =>
      formTemplateDataAccess.getFormTemplateGroups()
    );
    ...
    async function resetPagination(order = "", ordertype = "") {
      if (token && tenants_id) {
        ...
        if (order == "" && ordertype == "") {
          formTemplateDataAccess.resetFormTemplateGroup();
          await formTemplateDataAccess.fetchFormTemplateGroups(
            parseInt(tenants_id),
            filter.value,
            sort.value
          );
          pagination.value.total = formTemplateGroupsData.value.length;
    ...
    async function deleteTemplateGroup(templategroupid: number) {
      if (
        tenants_id &&
        confirm("Are you sure to delete this Form Template Group?")
      ) {
        ...
        toast.add({
          ...
          detail: "Form Template Group Delete Successfully",
          life: 10000,
        });
      }
    }
    ...
    const filteredTemplateGroups = computed(() => {
      let templateGroup: any = formTemplateGroupsData.value;
      let sort_Key: any = sortKey.value;
      let order: any = sortOrders.value[sort_Key] || 1;
      if (sortKey) {
        templateGroup = templateGroup.slice().sort((a: any, b: any) => {
    ...
    const paginatedTemplateGroups = computed(() => {
      return paginate(
    ...
    return {
      ...
      getIndex,
      filteredTemplateGroups,
      paginatedTemplateGroups,
      checked,
      deleteTemplateGroup,
    };
  },
});
export default FormTemplateGroupDashboard;
</script>
<template>
  <div class="dashboard-wrapper">
    <section>
      ...
      <div v-else>
        <SubHeaderAction>
          <template v-slot:actionsLeft> Form Template Group </template>
          <template v-slot:actionsRight>
            ...
            <router-link
              to="/formTemplates/groups/add"
              class="action datatable-router-link"
              v-tooltip.top="'Add'"
              v-if="$route.path == '/formTemplates/groups'"
            >
              <font-awesome-icon
                class="action-icon"
                :class="'add-icon'"
          ...
          <div class="datatable-tabs-container">
            <TabView>
              <TabPanel>
                ...
                >
                  <span class="datatable-tab-label">Groups</span>
                  <span class="datatable-tab-label-count">{{
                    formTemplateGroupsData.length
                  }}</span>
                </template>
                  ...
                  <div class="p-datatable-wrapper">
                    <table class="p-datatable-table">
                      ...
                      <tbody class="p-datatable-tbody">
                        <tr
                          v-for="templateGroup in paginatedTemplateGroups"
                          :key="templateGroup.id"
                        >
                          <td><Checkbox v-model="checked" :binary="true" /></td>
                          <!-- <td>{{ formTemplateGroupsData.id }}</td> -->
                          <td>
                            <router-link
                              :to="{
                                name: 'FormTemplateGroupView',
                                params: { id: templateGroup.id },
                              }"
                          ...
                          <td>{{ templateGroup.formTemplates.length }}</td>
                          <td>
                          ...
                          <td>
                            ...
                            <a
                              class="action-btn"
                              href="#"
                              @click="deleteTemplateGroup(templateGroup.id)"
                            >
